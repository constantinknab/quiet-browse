...
// Fast release check for manifest scope, packaged paths, JavaScript syntax, and
// forbidden executable patterns. The more detailed policy assertions live in tests/.
...
for (const path of [
  ...
  await stat(join(extension, path));
...
for (const path of all) {
  if (path.endsWith('.js')) {
    ...
    for (const pattern of [
      ...
      assert.ok(!pattern.test(source), `Forbidden executable pattern ${pattern} in ${path}`);
    const fetches = source.match(/\bfetch\s*\(/g) || [];
    if (path === join(extension, 'background.js'))
      assert.equal(
        fetches.length,
        1,
        'Only the fixed adult-list data download is allowed in the worker.',
      );
...
console.log(
  `PASS: manifest, disclosed permissions, exact required hosts, icons, local links, syntax, and static security checks (${all.length} extension files).`,
);
console.log(
  'This is a limited static check, not a security audit, legal opinion, or store approval.',
);
