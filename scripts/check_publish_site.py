...
class LocalLinkChecker(HTMLParser):
    """Report local links that are missing or escape the website directory."""

    def handle_starttag(self, tag, attrs):
        attributes_by_name = dict(attrs)
        for key in ("href", "src"):
            value = attributes_by_name.get(key, "")
...
for path in SITE.glob("*.html"):
    ...
    parser = LocalLinkChecker()
    parser.feed(source)
    if "<script" in source.lower():
        errors.append(f"Scripts are not expected in {path.name}")
    if any(
        term in source.lower()
        for term in ("google-analytics", "googletagmanager", "facebook pixel")
    ):
        errors.append(f"Tracking reference in {path.name}")
    if not ALLOW_PLACEHOLDERS and any(
        term in source for term in ("[PUBLISHER NAME]", "[SUPPORT EMAIL]", "SUPPORT_EMAIL")
    ):
...
for phrase in (
    ...
    if phrase not in normalized_privacy:
        errors.append(f"Privacy policy is missing: {phrase}")
