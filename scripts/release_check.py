...
for name in ["privacyPolicyUrl", "supportUrl"]:
    ...
    if url.scheme != "https" or not url.hostname or url.hostname in placeholder_hosts:
        missing.append(name + " (real public HTTPS URL)")
for name in [
    "liveChromeChecklistComplete",
    "liveYouTubeChecklistComplete",
    "publicPrivacyPolicyFinalized",
    "actualStoreScreenshotsAndPromoReady",
    "publisherPolicyAndLegalReviewComplete",
]:
...
if missing:
    ...
    raise SystemExit(2)
print(
    "Publisher checklist recorded. Verify the evidence and the current dashboard "
