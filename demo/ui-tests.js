...
status.id = 'ui-test-status'; status.setAttribute('role', 'status'); status.textContent = 'Running UI checks…';
const anchor = document.querySelector('main'); anchor.prepend(status, results);
let count = 0;
function assert(condition, label) {
  const item = document.createElement('li'); item.className = condition ? 'pass' : 'fail';
  item.textContent = `${condition ? 'PASS' : 'FAIL'} — ${label}`; results.append(item);
  if (!condition) throw new Error(label); count += 1;
}
try {
  if (location.pathname.includes('/popup')) {
    const parameters = new URLSearchParams(location.search);
    const testSite = parameters.has('social') ? 'https://www.instagram.com' : parameters.has('shopping') ? 'https://www.amazon.com' : 'https://www.youtube.com';
    await waitFor(() => !document.getElementById('enable').disabled);
    const enable = document.getElementById('enable'); enable.click();
    await waitFor(() => enable.textContent === 'Turn off for this site');
    assert(document.getElementById('navigation').disabled === false, 'Granting one simulated site enables its controls');
    const paging = document.getElementById('pageMode'); paging.click();
    await waitFor(() => paging.checked);
    const toggle = document.getElementById('gray-toggle');
    if (parameters.has('shopping')) {
      assert(toggle.getAttribute('aria-pressed') === 'true' && document.getElementById('gray-value').textContent === '20%', 'Built-in shopping profiles begin at 20% grayscale');
    } else {
      toggle.click(); await waitFor(() => toggle.getAttribute('aria-pressed') === 'true');
    }
    const level = document.getElementById('gray-level'); level.value = '42';
    level.dispatchEvent(new Event('input', { bubbles: true })); level.dispatchEvent(new Event('change', { bubbles: true }));
    ...
    assert(config.settings.grayscale.enabled && config.settings.grayscale.level === 42, 'Manual grayscale strength is saved for this host');
    ...
    if (new URLSearchParams(location.search).has('offline')) {
      ...
      assert(!document.getElementById('cover').hidden && document.getElementById('cover').disabled, 'Hide video picture stays visible while awaiting a page reload');
      assert(document.getElementById('cover-note').textContent.startsWith('Reload this YouTube page'), 'The unavailable hide-video control explains how to restore it');
    }
    ...
    assert(categories.map(group => group.querySelector('summary strong').textContent).join('|') === 'Social|Ecommerce|Other websites', 'Saved sites are grouped into Social, Ecommerce, and Other dropdowns');
    const ecommerce = categories[1];
    assert(!ecommerce.open && !ecommerce.querySelector('.site-card').open, 'The ecommerce list stays compact until its category and a site are opened');
    const social = categories[0]; social.open = true;
    const instagram = social.querySelector('.site-card'); instagram.open = true;
    const form = instagram.querySelector('form');
    const grayControl = form.querySelector('.control-block'); grayControl.open = true;
    grayControl.querySelector('input[type="checkbox"]').click();
    const grayMode = grayControl.querySelector('select'); grayMode.value = 'scheduled'; grayMode.dispatchEvent(new Event('change', { bubbles: true }));
    grayControl.querySelector('.windows-editor > button').click();
    const [start, end] = grayControl.querySelectorAll('input[type="time"]');
    start.value = '20:30'; start.dispatchEvent(new Event('input', { bubbles: true }));
    end.value = '06:45'; end.dispatchEvent(new Event('input', { bubbles: true }));
    const storiesControl = form.querySelector('.social-control'); storiesControl.open = true;
    const storiesMode = storiesControl.querySelector('select'); storiesMode.value = 'scheduled'; storiesMode.dispatchEvent(new Event('change', { bubbles: true }));
    storiesControl.querySelector('.windows-editor > button').click();
    form.requestSubmit();
    await waitFor(() => form.querySelector('.message').textContent === 'Saved.');
    const settings = (await chrome.runtime.sendMessage({ type: 'QB_LIST' })).data.sites['https://www.instagram.com'].settings;
    ...
    await waitFor(() => document.getElementById('adult-message').textContent.includes('saved for next time'));
    document.getElementById('adult-enable').click();
    await waitFor(() => !document.getElementById('adult-on').hidden);
    assert(document.getElementById('adult-status').textContent.includes('1 added'), 'Quit Porn activates the local blocker and an additional domain');
    assert(document.querySelectorAll('#adult-source-options-on input:checked').length === 3 && document.getElementById('adult-status').textContent.includes('4,542 downloaded'), 'US, China, and Japan lists show independent selected state and a combined active count');
    document.getElementById('adult-refresh').click();
    await waitFor(() => document.getElementById('adult-message').textContent.includes('Selected lists updated'));
    assert(!document.getElementById('adult-refresh').disabled, 'The selected regional lists can be refreshed manually');
    assert(!document.getElementById('adult-current-password').offsetParent, 'Password input stays hidden when password protection was not selected');
    document.getElementById('adult-disable').click();
    await waitFor(() => !document.getElementById('adult-off').hidden);
    assert(document.querySelectorAll('#adult-source-options-off input:checked').length === 3 && (await chrome.runtime.sendMessage({ type: 'QB_ADULT_STATUS' })).data.remoteSources.length === 3, 'Turning the blocker off removes active filtering but remembers list choices across settings reloads');
  }
