...
(() => {
  ...
  try {
    savedPolicy = JSON.parse(sessionStorage.getItem(policyKey));
  } catch {
    /* A malformed fixture value is ignored. */
  }
  const policy =
    savedPolicy?.enabled === true || savedPolicy?.enabled === false
      ? savedPolicy
      : {
          enabled: true,
          settings: {
  ...
  window.chrome = {
    runtime: {
      id: 'quiet-browse-test',
      onMessage: { addListener: (callback) => listeners.push(callback) },
      sendMessage: async () => ({ ok: true, data: structuredClone(policy) }),
    },
  };
  window.lab = {
    policy,
    ...
    send: (message) =>
      new Promise((resolve) =>
        listeners.forEach((listener) => listener(message, { id: 'quiet-browse-test' }, resolve)),
      ),
