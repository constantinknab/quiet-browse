...
(() => {
  ...
  const sourceTemplates = [
    {
      ...
      detail: 'Popular English-language and US-facing coverage.',
      limit: 1995,
      activeCount: 1995,
      count: 0,
      lastChecked: 0,
    ...
    },
    {
      ...
      detail: 'Chinese-community-maintained category coverage.',
      limit: 1900,
      activeCount: 1900,
      count: 0,
      lastChecked: 0,
    ...
    },
    {
      ...
      detail: 'Japanese-content and .jp coverage from V2Fly.',
      limit: 1500,
      activeCount: 647,
      count: 0,
      lastChecked: 0,
  ...
  let adult = {
    ...
    lastUpdated: 0,
    lastError: '',
    sources: sourceTemplates.map((source) => ({ ...source, selected: false })),
  };
  ...
  const page = {
    ...
    paused: false,
    covered: false,
    coverAvailable: !socialPreview && !shoppingPreview,
    loops: 0,
    choices: 0,
    videos: 0,
    recommendations: 0,
  ...
  window.chrome = {
    runtime: {
      ...
      openOptionsPage: async () => {
        window.location.href = '/demo/options.html';
      },
      sendMessage: async (message) => {
        ...
        if (message.type === 'QB_ADULT_PREFERENCES') {
          adult.remoteSources = [...message.sources];
          adult.remoteUpdates = false;
          adult.sources = adult.sources.map((source) => ({
            ...source,
            selected: message.sources.includes(source.id),
          }));
          return { ok: true, data: structuredClone(adult) };
        }
        if (message.type === 'QB_ADULT_ENABLE') {
          const selected = message.sources || [];
          const now = Date.now();
          adult = {
            ...adult,
            enabled: true,
            passwordProtected: !!message.password,
            customDomains: [...message.domains],
            remoteUpdates: !!selected.length,
            remoteSources: [...selected],
            remoteCount: selected.reduce(
              (sum, id) => sum + sourceTemplates.find((source) => source.id === id).activeCount,
              0,
            ),
            lastChecked: now,
            lastUpdated: selected.length ? now : 0,
            sources: sourceTemplates.map((source) => ({
              ...source,
              selected: selected.includes(source.id),
              count: selected.includes(source.id) ? source.activeCount : 0,
              lastChecked: selected.includes(source.id) ? now : 0,
              lastUpdated: selected.includes(source.id) ? now : 0,
            })),
          };
          return { ok: true, data: structuredClone(adult) };
        }
        if (message.type === 'QB_ADULT_UPDATE') {
          adult.customDomains = [...message.domains];
          return { ok: true, data: structuredClone(adult) };
        }
        if (message.type === 'QB_ADULT_AUTO') {
          adult.remoteSources = [...message.sources];
          adult.remoteUpdates = !!message.sources.length;
          adult.sources = adult.sources.map((source) => ({
            ...source,
            selected: message.sources.includes(source.id),
          }));
          adult.remoteCount = adult.sources
        ...
        if (message.type === 'QB_ADULT_REFRESH') {
          const now = Date.now();
          adult.lastChecked = now;
          adult.lastUpdated = now;
          adult.lastError = '';
          adult.sources = adult.sources.map((source) =>
            source.selected
              ? {
        ...
        if (message.type === 'QB_ADULT_DISABLE') {
          const selected = message.sources || adult.remoteSources;
          adult = {
            ...adult,
            enabled: false,
            passwordProtected: false,
        ...
        if (message.type === 'QB_SAVE') {
          ...
          if (site === 'https://www.youtube.com')
            page.covered = page.active && message.settings.youtubePictureCover === true;
    ...
    tabs: {
      ...
      query: async () => [{ id: 1, url: `${site}/watch?v=fixture` }],
      sendMessage: async (id, message) => {
        if (pageUnavailable)
          throw new Error('Could not establish connection. Receiving end does not exist.');
        if (message.type === 'QB_PAUSE') {
          page.paused = message.paused;
          page.active = !page.paused;
          if (page.paused) page.covered = false;
        }
