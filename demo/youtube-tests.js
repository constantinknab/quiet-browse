(async () => {
  ...
  try {
    ...
    let coverStatus = await send({ type: 'QB_REFRESH' }); await wait();
    assert(coverStatus.coverPersistent && coverStatus.covered && !!document.querySelector('[data-qb-cover]'), 'Saved picture cover applies without a page-only button press');
    ...
    assert(!coverStatus.covered && coverStatus.coverPersistent && coverStatus.coverTemporary && !document.querySelector('[data-qb-cover]'), 'Show picture creates a temporary override without changing the saved cover preference');
    coverStatus = await send({ type: 'QB_REFRESH' }); await wait();
    assert(!coverStatus.covered && !document.querySelector('[data-qb-cover]'), 'An unrelated settings refresh preserves the current-page Show picture override');
    window.dispatchEvent(new PopStateEvent('popstate')); await wait();
    coverStatus = await send({ type: 'QB_STATUS' });
    assert(coverStatus.covered && !coverStatus.coverTemporary && !!document.querySelector('[data-qb-cover]'), 'Browser back or forward navigation reapplies the saved cover');
    await send({ type: 'QB_COVER', covered: false });
    document.dispatchEvent(new Event('yt-navigate-finish')); await wait();
    coverStatus = await send({ type: 'QB_STATUS' });
    assert(coverStatus.covered && !coverStatus.coverTemporary && !!document.querySelector('[data-qb-cover]'), 'YouTube video navigation reapplies the saved cover');
    coverStatus = await send({ type: 'QB_PAUSE', paused: true });
    assert(!coverStatus.active && !coverStatus.covered && !document.querySelector('[data-qb-cover]'), 'Show original temporarily removes a saved cover');
    coverStatus = await send({ type: 'QB_PAUSE', paused: false }); await wait();
    assert(coverStatus.active && coverStatus.covered && !!document.querySelector('[data-qb-cover]'), 'Restoring Quiet Browse returns the saved cover');
    ...
    coverStatus = await send({ type: 'QB_REFRESH' }); await wait();
    assert(!coverStatus.coverPersistent && !coverStatus.covered && !document.querySelector('[data-qb-cover]'), 'Turning off the saved preference restores the picture immediately');
    coverStatus = await send({ type: 'QB_COVER', covered: true });
    assert(coverStatus.covered && coverStatus.coverTemporary && !!document.querySelector('[data-qb-cover]'), 'The original page-only cover remains available when persistence is off');
    await send({ type: 'QB_COVER', covered: false });
    assert(!document.querySelector('[data-qb-cover]'), 'The page-only Show picture action removes its cover');
    ...
    $('test-status').textContent = 'PASS — 22 YouTube adapter fixture checks. Live YouTube still needs manual verification.';
