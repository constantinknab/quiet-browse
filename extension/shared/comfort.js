// Shared pure logic. Valid both as a classic content script and an ES module.
(() => {
  const percent = (value, fallback = 100) => typeof value === 'number' && Number.isFinite(value)
  ...
  function cleanGrayscale(value) {
    ...
    for (const entry of Array.isArray(value?.windows) ? value.windows.slice(0, 12) : []) {
      ...
      const days = [...new Set((Array.isArray(entry.days) ? entry.days : []).filter(day => Number.isInteger(day) && day >= 0 && day <= 6))].sort();
      if (!days.length) continue;
      windows.push({ days, start: entry.start, end: entry.end, level: percent(entry.level) });
    }
    return { enabled: value?.enabled === true, level: percent(value?.level), scheduled: value?.scheduled === true, windows };
  }
  ...
  function windowActive(entry, date) {
    const start = minutes(entry.start), end = minutes(entry.end);
    if (start === null || end === null || !Array.isArray(entry.days) || !Number.isFinite(date.getTime())) return false;
    const day = date.getDay(), time = date.getHours() * 60 + date.getMinutes();
    ...
    return (entry.days.includes(day) && time >= start) || (entry.days.includes((day + 6) % 7) && time < end);
  }
  function grayscaleAt(value, date = new Date()) {
    ...
    return Math.max(0, ...gray.windows.filter(entry => windowActive(entry, date)).map(entry => entry.level));
  }
  ...
  function pageDistance(height, occlusion = 0) {
    ...
    const obstruction = Math.min(Math.max(0, Number(occlusion) || 0), Math.min(96, viewport * 0.12));
  ...
  function createWheelGate(gap = 320) {
    ...
    return {
      take(delta, now, mode = 0) {
        ...
        if (now - last > gap || sign !== direction) { amount = 0; moved = false; }
        last = now; direction = sign;
        amount += Math.abs(delta) * (mode === 1 ? 16 : mode === 2 ? 800 : 1);
        if (moved || amount < 8) return false;
        moved = true;
        return true;
      },
      reset() { last = -Infinity; direction = 0; amount = 0; moved = false; },
    };
  }
  Object.defineProperty(globalThis, 'QuietBrowseComfort', {
    value: Object.freeze({ percent, minutes, cleanGrayscale, cleanSchedule, windowActive, grayscaleAt, settingAt, pageDistance, createWheelGate }), configurable: true,
  });
})();
