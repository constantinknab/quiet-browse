...
// A controlled Chrome API fixture lets these tests exercise service-worker
// permissions, persistence, messaging, alarms, and blocking rules without reading
...
function fakeChrome() {
  ...
  const chrome = {
    ...
    permissions: {
      ...
      remove: async ({ origins }) => {
        origins.forEach((origin) => grants.delete(origin));
        return true;
      },
    },
    ...
    tabs: {
      query: async () => [{ id: 1 }],
      get: async (id) => ({ id, url: 'https://example.com/a?private=yes' }),
  ...
  return {
    ...
    setFailInjection: (value) => {
      failInjection = value;
    },
    setFailDynamicUpdate: (value) => {
      failDynamicUpdate = value;
    },
  };
}
...
function completeFeatureSettings(value) {
  ...
  settings.grayscale = value
    ? {
        ...
        level: 73,
        scheduled: true,
        windows: [{ days: [1, 3, 5], start: '21:15', end: '06:45', level: 84 }],
      }
    : { enabled: false, level: 11, scheduled: false, windows: [] };
  settings.socialSchedules = Object.fromEntries(
    SOCIAL_SCHEDULE_KEYS.map((key, index) => [
      key,
      value
        ? {
            scheduled: true,
...
test('background permission, scope, messaging, persistence and revocation lifecycle', async (testContext) => {
  ...
  const ui = { id: fixture.chrome.runtime.id, url: fixture.chrome.runtime.getURL('ui/popup.html') };
  const page = {
  ...
  const send = (message, sender = ui) =>
    new Promise((resolve) =>
      fixture.chrome.runtime.onMessage.listeners[0](message, sender, resolve),
    );

  await testContext.test(
    'a content script cannot enable a site or read other saved sites',
    async () => {
      assert.equal(
        (await send({ type: 'QB_SAVE', site: 'https://example.com', enabled: true }, page)).ok,
        false,
      );
      assert.equal((await send({ type: 'QB_LIST' }, page)).ok, false);
    },
  );
  await testContext.test('sender identity and extension-page URLs cannot be forged', async () => {
    ...
    assert.equal(
      (await send({ type: 'QB_LIST' }, { id: 'different-extension', url: ui.url })).ok,
      false,
    );
    assert.equal(
      (
        await send(
          { type: 'QB_LIST' },
          { id: fixture.chrome.runtime.id, url: `${ui.url}.lookalike` },
        )
      ).ok,
      false,
    );
  ...
  await testContext.test('enable requires an already granted exact host permission', async () => {
    assert.equal(
      (await send({ type: 'QB_SAVE', site: 'https://example.com', enabled: true })).ok,
      false,
    );
    assert.equal(fixture.registered.size, 0);
  });
  await testContext.test(
    'enable persists only preferences, registers the host, and injects the current tab',
    async () => {
  ...
  await testContext.test('policy scope comes from the sender, not caller input', async () => {
    assert.equal(
      (await send({ type: 'QB_POLICY', site: 'https://evil.test' }, page)).data.enabled,
      true,
    );
    assert.equal(
      (
        await send(
          { type: 'QB_POLICY', site: 'https://example.com' },
  ...
  await testContext.test('new content scripts migrate an existing registration', async () => {
    ...
    assert.deepEqual([...fixture.registered.values()][0].js, [
      'shared/comfort.js',
      'content/comfort.js',
      'content/social.js',
      'content/engine.js',
  ...
  await testContext.test(
    'an outdated live page is replaced before newly saved settings are applied',
    async () => {
      const before = fixture.injected.filter((entry) => entry.files).length;
      fixture.setReceiver(true, 8);
      const response = await send({
        type: 'QB_SAVE',
        site: 'https://example.com',
        enabled: true,
      ...
      assert.equal(response.ok, true);
      assert.equal(response.data.pageReady, true);
      assert.equal(fixture.injected.filter((entry) => entry.files).length, before + 1);
      assert.equal((await send({ type: 'QB_POLICY' }, page)).data.settings.pageMode, true);
    },
  );
  await testContext.test('a missing receiver is cleaned up and reinjected', async () => {
    fixture.setReceiver(false);
    const response = await send({
      type: 'QB_SAVE',
      site: 'https://example.com',
      enabled: true,
    ...
    assert.equal(response.ok, true);
    assert.equal(response.data.pageReady, true);
    assert.ok(fixture.injected.some((entry) => typeof entry.func === 'function'));
  });
  await testContext.test(
    'a page injection failure never rolls back or reports a failed preference save',
    async () => {
      fixture.setReceiver(false);
      fixture.setFailInjection(true);
      const response = await send({
        type: 'QB_SAVE',
        site: 'https://example.com',
        enabled: true,
      ...
      fixture.setFailInjection(false);
      assert.equal(response.ok, true);
      assert.equal(response.data.pageReady, false);
      assert.equal((await send({ type: 'QB_POLICY' }, page)).data.settings.pageMode, false);
      fixture.setReceiver(true);
    },
  );
  await testContext.test(
    'trusted options query URLs can save schedules; lookalike URLs cannot',
    async () => {
      const schedule = {
        enabled: true,
        scheduled: true,
        windows: [{ days: [1], start: '21:00', end: '07:00', level: 75 }],
      };
      const options = {
        id: fixture.chrome.runtime.id,
        url: fixture.chrome.runtime.getURL('ui/options.html?site=https%3A%2F%2Fexample.com'),
      };
      assert.equal(
        (
          await send(
      ...
      assert.equal(fixture.alarms.size, 1);
      assert.equal(fixture.alarms.get('qb-schedule-clock').periodInMinutes, 1);
      assert.deepEqual(
        (await send({ type: 'QB_POLICY' }, page)).data.settings.grayscale.windows,
        schedule.windows,
      );
      assert.equal(
        (
          await send(
            { type: 'QB_LIST' },
            { ...options, url: options.url.replace('options.html?', 'options.html.evil?') },
          )
        ).ok,
        false,
      );
  ...
  await testContext.test(
    'manual grayscale and empty schedules need no background alarm',
    async () => {
      for (const grayscale of [
        { enabled: true, scheduled: false, level: 65 },
        { enabled: true, scheduled: true, windows: [] },
      ]) {
        await send({
          type: 'QB_SAVE',
          site: 'https://example.com',
  ...
  await testContext.test(
    'turning off unregisters future execution and notifies loaded pages',
    async () => {
      ...
      assert.ok(fixture.messages.some((sentMessage) => sentMessage.message.type === 'QB_REFRESH'));
    },
  );
  await testContext.test('removing a site deletes data and its optional permission', async () => {
    assert.equal((await send({ type: 'QB_FORGET', site: 'https://example.com' })).ok, true);
    assert.equal(fixture.grants.size, 0);
    assert.equal(Object.keys(fixture.data()[STATE_KEY].sites).length, 0);
  });
  await testContext.test(
    'Chrome-side permission revocation disables saved configuration',
    async () => {
      fixture.grants.add('https://example.com/*');
  ...
  await testContext.test('reset clears every saved host and optional permission', async () => {
    fixture.grants.add('https://example.com/*');
    await send({
      type: 'QB_SAVE',
      site: 'https://example.com',
      ...
      settings: {
        grayscale: {
          enabled: true,
          scheduled: true,
          windows: [{ days: [0], start: '00:00', end: '00:00', level: 100 }],
        },
      },
    });
    assert.equal(fixture.alarms.size, 1);
  ...
  await testContext.test(
    ...
    async (testContext) => {
      ...
      for (const site of sites) {
        await testContext.test(new URL(site).hostname, async () => {
          const page = {
            id: fixture.chrome.runtime.id,
            tab: { id: 1 },
            frameId: 0,
          ...
          assert.equal(
            (await send({ type: 'QB_SAVE', site, enabled: true, settings: allOn })).ok,
            true,
          );
          ...
          assert.ok(
            [...fixture.registered.values()].some(
  ...
  await testContext.test(
    'adult-site rules are local, password protected, editable and reset only with the password',
    async () => {
      const enabled = await send({
        type: 'QB_ADULT_ENABLE',
        domains: ['custom.example', 'custom.example'],
      ...
      assert.equal(fixture.dynamicRules.size, enabled.data.packagedCount + 1);
      assert.ok(
        [...fixture.dynamicRules.values()].some(
          (rule) => rule.condition.urlFilter === '||custom.example^',
        ),
      );
      for (const rule of fixture.dynamicRules.values()) {
        assert.deepEqual(rule.action, { type: 'block' });
        assert.deepEqual(rule.condition.resourceTypes, ['main_frame']);
      }
      assert.equal(JSON.stringify(fixture.data()).includes('shared passphrase'), false);
      assert.equal(
        (
          await send({
      ...
      assert.equal(
        ...
        ).data.customDomains[0],
        'changed.example',
      );
      assert.equal((await send({ type: 'QB_RESET', password: 'shared passphrase' })).ok, true);
      assert.equal(fixture.dynamicRules.size, 0);
    },
  );
  await testContext.test(
    'three bounded regional lists are parsed independently, clear retired rules, and remain last-known-good',
    async () => {
      const lists = {
        adblock:
          '[Adblock Plus]\n! generated fixture\n' +
      ...
      fixture.grants.add(ADULT_LIST_PERMISSION);
      fixture.dynamicRules.set(200000, {
        id: 200000,
        priority: 1,
        action: { type: 'block' },
        condition: { urlFilter: '||retired.example^', resourceTypes: ['main_frame'] },
      });
      const enabled = await send({
        type: 'QB_ADULT_ENABLE',
        domains: [],
        password: '',
        sources: ADULT_SOURCES.map((source) => source.id),
      });
      assert.equal(enabled.ok, true);
      ...
      assert.deepEqual(
        enabled.data.sources.map((source) => source.count),
        [1995, 1900, 800],
      );
      assert.equal(fixture.dynamicRules.has(200000), false);
      assert.equal(fixture.dynamicRules.size, enabled.data.packagedCount + 1995 + 1900 + 800);
      const remoteDomains = [...fixture.dynamicRules.values()]
        .filter((rule) => rule.id >= 200000)
        .map((rule) => rule.condition.urlFilter.slice(2, -1));
      ...
      assert.equal(fixture.alarms.get('qb-adult-list-update').periodInMinutes, 7 * 24 * 60);
      assert.equal(JSON.stringify(fixture.data()).includes('us1000.example'), false);

      const remoteRuleIds = [...fixture.dynamicRules.keys()].filter((id) => id >= 200000);
      await send({ type: 'QB_ADULT_UPDATE', domains: ['custom.example'], password: '' });
      assert.deepEqual(
        [...fixture.dynamicRules.keys()].filter((id) => id >= 200000),
        remoteRuleIds,
      );
      ...
      globalThis.fetch = async (url) => {
        const source = ADULT_SOURCES.find((item) => item.url === String(url));
        const content =
          source.format === 'adblock' ? 'not-an-adblock-row.example' : lists[source.format];
      ...
      fixture.setFailDynamicUpdate(true);
      const rejected = await send({ type: 'QB_ADULT_REFRESH' });
      fixture.setFailDynamicUpdate(false);
      ...
      assert.equal(fixture.grants.has(ADULT_LIST_PERMISSION), false);
      assert.deepEqual(
        [...fixture.dynamicRules.keys()].filter((id) => id >= 200000),
        [],
      );
      ...
      const changedWhileOff = await send({
        type: 'QB_ADULT_PREFERENCES',
        sources: [ADULT_SOURCES[2].id],
      });
      assert.deepEqual(changedWhileOff.data.remoteSources, [ADULT_SOURCES[2].id]);
      assert.deepEqual((await send({ type: 'QB_ADULT_STATUS' })).data.remoteSources, [
        ADULT_SOURCES[2].id,
      ]);
      assert.equal(fixture.dynamicRules.size, 0);
    },
        ...
        [STATE_KEY]: {
          ...
          sites: {
            'https://www.amazon.com': {
              enabled: true,
              settings: { grayscale: { enabled: true, level: 47 } },
            },
