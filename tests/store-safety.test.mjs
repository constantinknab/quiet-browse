...
const reviewableTextFiles = extensionFiles.filter((path) =>
...
test('declared Chrome permissions are narrow, known, and used by current features', async () => {
  ...
  assert.deepEqual(
    manifest.permissions.filter((permission) => forbidden.includes(permission)),
    [],
  );
...
test('page-facing scripts have no network or telemetry capability', async () => {
  ...
  const packageText = (
    await Promise.all(
      extensionFiles.filter((path) => ['.js', '.html', '.json'].includes(extname(path))).map(text),
    )
  ).join('\n');
  for (const marker of [
...
test('packaged source preserves readable formatting, names, comments, and module boundaries', async () => {
  const focusedModules = [
    'background.js',
    'shared/settings.js',
    'shared/comfort.js',
    'shared/adult-domains.js',
    'content/comfort.js',
    'content/social.js',
    'content/engine.js',
    'ui/options.js',
  ...
  const packagedPaths = new Set(extensionFiles.map((path) => relative(extensionRoot, path)));
  for (const modulePath of focusedModules) {
    assert.ok(packagedPaths.has(modulePath), `focused module is missing: ${modulePath}`);
  }

  // JavaScript, HTML, and CSS stay in separately reviewable files. A generous
  // ceiling catches a future collapsed bundle without limiting ordinary modules.
  for (const path of javascriptFiles) {
    const source = await text(path);
    const lines = source.split(/\r?\n/);
    const readablePath = relative(projectRoot, path);

    assert.ok(source.trimStart().startsWith('//'), `${readablePath} needs a purpose comment`);
    assert.ok(lines.length <= 1000, `${readablePath} needs to be split by responsibility`);

    // One-character declarations and parameters are hard for a reviewer to
    // follow and are also a common result of name-minifying tools.
    assert.doesNotMatch(
      source,
      /\b(?:const|let|var)\s+[A-Za-z]\b/,
      `single-letter declaration found in ${readablePath}`,
    );
    assert.doesNotMatch(
      source,
      /(?:\(|,)\s*[A-Za-z]\s*(?:,|\))\s*=>/,
      `single-letter arrow parameter found in ${readablePath}`,
    );
    assert.doesNotMatch(source, /\bconst\s+\$\s*=/, `cryptic DOM alias found in ${readablePath}`);
  ...
  // This limit applies to every code-bearing text format, not just JavaScript.
  // It catches minified HTML/CSS and dense JSON while allowing URLs and copy.
  for (const path of reviewableTextFiles) {
    const source = await text(path);
    const longestLine = Math.max(...source.split(/\r?\n/).map((line) => line.length));
    assert.ok(longestLine <= 180, `${relative(projectRoot, path)} contains a packed line`);
  }

  for (const path of extensionFiles.filter((path) => extname(path) === '.css')) {
    assert.ok((await text(path)).trimStart().startsWith('/*'), `${path} needs a purpose comment`);
  }
  for (const path of htmlFiles) {
    assert.match(
      (await text(path)).split(/\r?\n/).slice(0, 4).join('\n'),
      /<!--.+-->/,
      `${path} needs a purpose comment near the top`,
    );
  }
});

test('supporting website, fixture, test, and release scripts remain readable too', async () => {
  const supportingRoots = ['demo', 'website', 'tests', 'scripts'].map((directory) =>
    join(projectRoot, directory),
  );
  const supportingFiles = (await Promise.all(supportingRoots.map(walk)))
    .flat()
    .filter((path) => ['.css', '.html', '.js', '.mjs', '.py'].includes(extname(path)));
  ...
  for (const path of supportingFiles) {
    ...
    const lines = source.split(/\r?\n/);
    assert.ok(
      Math.max(...lines.map((line) => line.length)) <= 180,
      `${readablePath} contains a packed line`,
    );

    if (['.js', '.mjs'].includes(extname(path))) {
      const openingSection = lines.slice(0, 25).join('\n');
      assert.match(openingSection, /\/\//, `${readablePath} needs an opening purpose comment`);
      assert.doesNotMatch(
        source,
        /\b(?:const|let|var)\s+[A-Za-z]\b/,
        `single-letter declaration found in ${readablePath}`,
    ...
    if (extname(path) === '.html') {
      assert.match(
        ...
        /<!--.+-->/,
        `${readablePath} needs a purpose comment near the top`,
      );
    }
    if (extname(path) === '.py') {
      assert.ok(
        source.trimStart().startsWith('"""'),
        `${readablePath} needs an opening module docstring`,
      );
    }
  }
});
...
test('sensitive browser databases and direct storage access stay out of page scripts', async () => {
  ...
  const worker = await text(join(extensionRoot, 'background.js'));
  assert.ok(
    worker.includes("setAccessLevel({ accessLevel: 'TRUSTED_CONTEXTS' })"),
    'local storage must remain hidden from content-script contexts',
  );
  for (const path of javascriptFiles.filter(
    (path) => path !== join(extensionRoot, 'background.js'),
  )) {
    assert.doesNotMatch(
      await text(path),
      /chrome\.storage\./,
...
test('public disclosures stay aligned with the package behavior and version', async () => {
  ...
  for (const policy of [publicPrivacy, packagedPrivacy]) {
    ...
    // this assertion checks the disclosure wording rather than its indentation.
    const normalizedPolicy = policy.replace(/\s+/g, ' ');
    assert.ok(
      normalizedPolicy.includes(`Version ${manifest.version}`) ||
        normalizedPolicy.includes(`VERSION ${manifest.version}`),
    );
    for (const fact of [
      'PBKDF2-SHA-256',
  ...
  assert.ok(
    normalizedPublicPrivacy.includes(
      'GitHub and its infrastructure can receive ordinary connection information',
    ),
  );
...
test('the release package boundary excludes secrets, hidden files, and unknown formats', async () => {
  ...
  for (const path of packageFiles) {
    ...
    assert.equal(
      info.isSymbolicLink(),
      false,
      `symlink would enter package: ${relative(extensionRoot, path)}`,
    );
    assert.ok(
      allowedExtensions.has(extname(path)),
      `unknown package format: ${relative(extensionRoot, path)}`,
    );
    assert.ok(
      info.size < 1024 * 1024,
      `unexpectedly large package file: ${relative(extensionRoot, path)}`,
    );
  ...
  assert.equal(
    await text(join(extensionRoot, 'LICENSE.txt')),
    await text(join(projectRoot, 'LICENSE')),
  );
