...
import { cleanSettings, cleanState, cleanGrayscale, cleanSchedule, grayscaleAt, settingAt } from '../extension/shared/settings.js';
...
test('old preferences migrate with paging and grayscale off and no shared mutable defaults', () => {
  const original = { version: 1, sites: { 'https://example.com': { enabled: true, settings: { motion: false } } } };
  ...
  assert.equal(b.grayscale.windows.length, 0);
  assert.equal(b.socialSchedules.socialStories.windows.length, 0);
  assert.equal(original.version, 1);
});
test('grayscale sanitizes strengths, flags, days, malformed times and unknown data', () => {
  const clean = cleanGrayscale({ enabled: 'yes', scheduled: true, level: 105, token: 'secret', windows: [
    { days: [1, 1, 6, -1, 7, '2'], start: '22:00', end: '07:00', level: -5, history: ['private'] },
    { days: [2], start: '24:00', end: '07:00' }, { days: [], start: '10:00', end: '11:00' }, null,
  ] });
  assert.deepEqual(clean, { enabled: false, scheduled: true, level: 100, windows: [{ days: [1, 6], start: '22:00', end: '07:00', level: 0 }] });
  assert.equal(cleanGrayscale({ level: 44.8 }).level, 45);
  assert.equal(cleanGrayscale({ level: NaN }).level, 100);
  assert.equal(cleanGrayscale({ windows: Array(20).fill({ days: [1], start: '01:00', end: '02:00' }) }).windows.length, 12);
});
test('manual amount covers both endpoints; master off overrides a schedule', () => {
  ...
  assert.equal(grayscaleAt({ enabled: false, scheduled: true, windows: [{ days: [1], start: '00:00', end: '00:00', level: 100 }] }, monday(12)), 0);
});
...
test('overlaps use the highest strength and empty schedules do nothing', () => {
  const a = { days: [1], start: '10:00', end: '14:00', level: 50 };
  const b = { days: [1], start: '12:00', end: '16:00', level: 100 };
  assert.equal(grayscaleAt(config([a, b]), monday(12)), 100);
  assert.equal(grayscaleAt(config([b, a]), monday(12)), 100);
  assert.equal(grayscaleAt(config([a, b]), monday(11)), 50);
  assert.equal(grayscaleAt(config([]), monday(12)), 0);
  assert.equal(grayscaleAt(config([a]), new Date(NaN)), 0);
});
test('social schedules use the same local-time and overnight rules', () => {
  const schedule = cleanSchedule({ scheduled: true, windows: [{ days: [1], start: '21:00', end: '07:00', ignored: 'private' }] });
  assert.deepEqual(schedule, { scheduled: true, windows: [{ days: [1], start: '21:00', end: '07:00' }] });
