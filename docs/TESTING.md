...
Updated September 9, 2026. Tests are intentionally split by what they establish. Version 0.5.7 is public in the Chrome Web Store; version 1.0.0 is the publisher-approved GitHub release. Complete the live Chrome checks below before uploading that ZIP to Google.
...
Observed 1.0.0 release result: **66 reported tests passed, zero failed** (including 12 per-site lifecycle cases and the parent lifecycle test). Rerun after any changes.
...
- `/demo/youtube-tests.html`: 30 checks covering independent Shorts-shelf and watch-page recommendation controls, Shorts-only and mixed shelf boundaries, ordinary recommendation and direct-link preservation, dynamically inserted Shorts shelves with the control on and off, previews, saved and page-only picture-cover modes, temporary override persistence, YouTube and browser-history navigation reapplication, pause/restore, native mute hit-testing and audio state, fixture caption/control stacking, recognized ad states, and undo.
...
- `/demo/popup-tests.html`, its direct-repair, stale-engine replacement, reload-fallback, Instagram, TikTok, and shopping scenarios, and `/demo/options-tests.html`: 67 checks covering site enablement, direct missing-receiver repair, automatic older-engine replacement, safe fallback, separate YouTube Shorts and saved/page-only picture-cover controls, exact platform-specific social controls, the 20% shopping default, Social/Video/Ecommerce/Other site dropdowns, grayscale and social overnight-window editing, the three-source adult-content-filter flow, and list-choice persistence after shutdown.

**Observed 1.0.0 release results: all 228 checks passed in both the in-app Chromium browser and the automated isolated headless-Chrome gate.** Fixtures use real DOM/CSS/animation/media elements under a content security policy that forbids inline scripts and inline styles. Chrome messaging, permissions, fetch, and declarativeNetRequest are simulated; the YouTube and social fixtures use local markup and test-only host adapters. This does not prove compatibility with live platforms, the live lists, or a loaded Chrome extension.

The additional lifecycle matrix at `/demo/lifecycle-<profile>.html` runs the production content controllers against local structural fixtures for Instagram, Facebook, TikTok, all eight built-in shopping hosts, and YouTube. Each profile tests every feature alone, unsupported-feature isolation, all features together, full restoration after the master switch is disabled, a reload while disabled with all settings retained, re-enabling without resubmitting those settings, a reload while enabled, dynamic content while disabled, scheduled grayscale and social activation, and final restoration. It performs **375 assertions per profile across two real document reloads (4,500 total)**. The observed September 9, 2026 result was all 12 profiles passing in the in-app browser and the automated isolated headless-Chrome gate. These are deterministic adapter tests with simulated policy messaging, not live-site or installed-extension results.
...
- [ ] On a fresh 1.0.0 install, verify the 12 disclosed profiles are seeded once. Upgrade from 0.5.7 and verify existing choices remain intact. Remove one, restart Chrome, and confirm it stays removed.
...
- [ ] Preview hiding, recommendation reveal, normal links, comments, and search remain functional. On YouTube home, verify supported Shorts shelves are hidden by default, ordinary recommendation shelves remain, direct Shorts links still open, and the dedicated switch restores only the Shorts shelves.
