...
Date: August 31, 2026. Tests are intentionally split by what they establish. The publisher reports personal beta testing, but no structured live Chrome permission, Web Store review, or live YouTube checklist was completed in this build session.
...
- `/demo/social-tests.html`: 23 checks covering Instagram route classification, independent Stories/follow-recommendation/Reels/Explore/home controls, recognition of current unlabeled button-driven Stories carousels without hiding post image carousels, the interaction between restored followed posts and still-hidden Stories/suggestions, local-time Stories scheduling, directly opened Story/Reel preservation, message/profile preservation, continuation-feed removal, notices, and complete undo.
...
- [ ] Instagram, Facebook, and TikTok signed-out and authorized signed-in layouts. Toggle Stories, follow recommendations, short-video, Explore/Discover, and home-feed controls one at a time. On Instagram, restore only the home feed and confirm Stories and follow recommendations remain hidden. On TikTok, verify that `/` is stopped by either the short-video or home-feed hide control and returns only when both applicable controls allow it.
- [ ] Give Stories, follow recommendations, short video, Explore, and the home feed separate schedules. Verify each surface changes at its own local-time boundary without changing messages or direct items.
