...
Run the tests and static check, then `python3 scripts/package.py`. The generated archive is `dist/quiet-browse-0.5.2.zip`, with `manifest.json` at its root. Only extension assets are included; fixtures and developer tools are excluded. The packager validates file paths and writes a SHA-256 checksum.
